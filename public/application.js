...
var showAccountManagementTemplate = __webpack_require__(19);
var showItemIndexTemplate = __webpack_require__(36);
var showItemTemplate = __webpack_require__(37);
var itemFailureTemplate = __webpack_require__(38);
var showCartTemplate = __webpack_require__(39);
var checkoutTemplate = __webpack_require__(40);
var adminDashboardTemplate = __webpack_require__(41);

var accountManagement = function accountManagement(data) {
  data.forEach(function (purchase) {
    var date = new Date(purchase.createdAt);
    purchase.createdAt = date.toLocaleDateString() + ' at ' + date.toLocaleTimeString();
    purchase.totalPrice = purchase.totalPrice.toFixed(2);
    purchase.products.forEach(function (i) {
      i[0].price = (i[1] * i[0].price).toFixed(2);
    });
  });

  var showAccountManagementHtml = showAccountManagementTemplate({ purchases: data });
...
var showItemIndex = function showItemIndex(data) {
  data.forEach(function (i) {
    i.price = i.price.toFixed(2);
  });
  var showItemIndexHtml = showItemIndexTemplate({ products: data });
  showContent(showItemIndexHtml);
};

var showItemSmall = function showItemSmall(data) {
  data.price = data.price.toFixed(2);
  var showItemSmallHtml = showItemTemplate({ product: data });
  showItemView(showItemSmallHtml);
};

var showCartView = function showCartView() {
  var data = store.user.cart;
  data.forEach(function (i) {
    i[0].price = (i[0].price * i[1]).toFixed(2);
  });
  data.forEach(function (i) {
    i[0].quantity = i[1];
  });
  var cart = data.map(function (i) {
    return i[0];
  });
  var showCartHtml = showCartTemplate({ cart: cart, price: store.user.cartItemPrice, total: store.user.cartItemTotal });
  showItemView(showCartHtml);
};

var checkoutView = function checkoutView() {
  var data = store.user.cart;
  var cart = data.map(function (i) {
    return i[0];
  });
  var checkoutHtml = checkoutTemplate({ cart: cart, price: store.user.cartItemPrice, total: store.user.cartItemTotal });
  showItemView(checkoutHtml);
};

var showContent = function showContent(data) {
  $('#content').empty();
  $('#content').append(data);
};

var itemFailure = function itemFailure() {
  showItemView(itemFailureTemplate());
};

var showItemView = function showItemView(data) {
  $('#item-view').empty();
...
var passwordChangeToggle = function passwordChangeToggle() {
  var hidden = $('#change-password-area').hasClass('hidden');
...
var hidePasswordChange = function hidePasswordChange(event) {
  event.preventDefault();
  $('#content #change-password-area').addClass('hidden');
};

var clearUpdatePassword = function clearUpdatePassword() {
  $('#change-password-form :input').val('');
};

var productIdToggle = function productIdToggle(event) {
  // check to see if the product update area is out
  if ($('#edit-product-area').hasClass('hidden')) {
    var hidden = $('#product-id-area').hasClass('hidden');
...
var product2IdToggle = function product2IdToggle(event) {
  var hidden = $('#product-id-area2').hasClass('hidden');
...
var addProductToggle = function addProductToggle(event) {
  var hidden = $('#add-product-area').hasClass('hidden');
...
var productUpdateToggle = function productUpdateToggle(event) {
  var hidden = $('#edit-product-area').hasClass('hidden');
...
var populateProduct = function populateProduct(product) {
...
var productLoadFailure = function productLoadFailure() {
...
var productUpdateSuccess = function productUpdateSuccess() {
  $('#alert-modal-content').addClass('alert-success');
  $('#alert-modal-content').html('<p>Product updated!</p>');
  $('#alertModal').modal('show');
};

var productUpdateFailure = function productUpdateFailure() {
  $('#alert-modal-content').addClass('alert-danger');
...
var addFailure = function addFailure() {
  $('#alert-modal-content').addClass('alert-danger');
...
module.exports = {
  passwordChangeToggle: passwordChangeToggle,
  hidePasswordChange: hidePasswordChange,
  clearUpdatePassword: clearUpdatePassword,
  productIdToggle: productIdToggle,
  productUpdateToggle: productUpdateToggle,
  populateProduct: populateProduct,
  productLoadFailure: productLoadFailure,
  productUpdateSuccess: productUpdateSuccess,
  productUpdateFailure: productUpdateFailure,
  product2IdToggle: product2IdToggle,
...
var authApi = __webpack_require__(17);
var authUI = __webpack_require__(18);
var itemApi = __webpack_require__(42);
var itemUI = __webpack_require__(43);
...
var signOutUser = function signOutUser(event) {
  event.preventDefault();
  authApi.signOut().then(authUI.signOutSuccess).then(loadItemIndex).catch(authUI.signOutFailure);
};

var formLoginAction = function formLoginAction(event) {
  event.preventDefault();
  var newuser = $('input[data-newuser]').prop('checked');
  var p = $('input[name="credentials[password]"]').val();
  var c = $('input[name="credentials[password_confirmation]"]').val();
...
var showPasswordConfirmation = function showPasswordConfirmation(event) {
  $('#signInComment').empty();
...
var loadItemIndex = function loadItemIndex() {
  // event.preventDefault()
  itemApi.indexItems().then(itemUI.onIndexSuccess).catch(itemUI.onIndexFailure);
};

var clearAlertModal = function clearAlertModal() {
...
var showItem = function showItem(event) {
  ...
  itemApi.getItem(productId).then(itemUI.onGetSuccess).catch(itemUI.onGetFailure);
};

var qtyChange = function qtyChange(event) {
  var quantity = $(event.target).val();
  var price = $('span[data-price]').attr('data-price');
  var totalPrice = quantity * price;
  $('span[data-total-price]').text('$' + totalPrice.toFixed(2));
};

var getUserCart = function getUserCart() {
  authApi.getCart().then(authUI.onGetCartSuccess).catch(authUI.onGetCartFailure);
};

var addToCart = function addToCart(event) {
  if (store.item && store.user) {
    var inCart = store.user.cart.find(function (i) {
      return i[0].id === store.item.product.id;
    });
    if (inCart) {
      var addQuantity = +store.item.qty + +inCart[1];
      authApi.removeCartItem(store.item.product.id).then(function (data) {
        return itemApi.getItem(store.item.product.id);
      }).then(function (data) {
        return promiseAddCart(data.product, addQuantity);
      }).then(authApi.addToCart).then(authApi.getCart).then(authUI.onGetCartSuccess).then(function () {
        store.item = null;
      }).catch(itemUI.onGetFailure);
    } else {
      itemApi.getItem(store.item.product.id).then(function (data) {
        return promiseAddCart(data.product, store.item.qty);
      }).then(authApi.addToCart).then(authApi.getCart).then(authUI.onGetCartSuccess).then(function () {
        store.item = null;
      }).catch(itemUI.onGetFailure);
    }
  } else {
    var prodID = $(event.target).closest('button').attr('data-prodID');
    var input = $(event.target).closest('button').siblings('input');
    var quantity = $(input).val() || 1;
    if (store.user) {
      var _inCart = store.item ? store.user.cart.some(function (i) {
        return i[0].id === store.item.product.id;
      }) : store.user.cart.some(function (i) {
        return i[0].id === prodID;
      });
      if (_inCart) {
        var _addQuantity = +store.user.cart.find(function (i) {
          return i[0].id === prodID;
        })[1] + +quantity;
        authApi.removeCartItem(prodID).then(function (data) {
          return itemApi.getItem(prodID);
        }).then(function (data) {
          return promiseAddCart(data.product, _addQuantity);
        }).then(authApi.addToCart).then(authApi.getCart).then(authUI.onGetCartSuccess).catch(itemUI.onGetFailure);
      } else {
        itemApi.getItem(prodID).then(function (data) {
          return promiseAddCart(data.product, quantity);
        }).then(authApi.addToCart).then(authApi.getCart).then(authUI.onGetCartSuccess).catch(itemUI.onGetFailure);
      }
    } else {
      $('#alert-modal-content').addClass('alert-danger');
...
var emptyCart = function emptyCart() {
  authApi.clearCart().then(authApi.getCart).then(authUI.onGetCartSuccess).catch(authUI.onGetCartFailure);
};

var removeFromCart = function removeFromCart(event) {
  var itemId = $(event.target).attr('data-delete');

  authApi.removeCartItem(itemId).then(authApi.getCart).then(authUI.onGetCartSuccess).catch(authUI.onGetCartFailure);
};

var updateQuantity = function updateQuantity(event) {
  var itemId = $(event.target).attr('data-update');
  var quantity = $('input[data-quantity="' + itemId + '"]').val();

  authApi.removeCartItem(itemId).then(function (data) {
...
var promiseAddCart = function promiseAddCart(product, qty) {
...
var checkout = function checkout(event) {
  ...
  var goodCard = data.purchase.card.length === 16;
  var goodExp = data.purchase.exp.length === 5;
  var goodCVC = data.purchase.cvc.length === 3;

  if (goodCard && goodExp && goodCVC) {
    authApi.buyCart().then(authUI.onPurchaseSuccess).catch(authUI.onPurchaseFailure);
  } else {
    if (!goodCard) {
...
var keyIgnores = [93, 91, 17, 18, 20, 16, 9, 13, 39, 38, 37, 40];

var cardExpHelper = function cardExpHelper(event) {
  ...
  var keyCheck = keyIgnores.some(function (i) {
    return i === event.which;
  });
  ...
  if (val.length === 5) {
    var date = new Date('20' + val.substr(-2), val.substr(0, 1));
    if (date < new Date()) {
...
var cardHelper = function cardHelper(event) {
...
var cardCVCHelper = function cardCVCHelper(event) {
...
var loadAccountManagement = function loadAccountManagement() {
  authApi.getPurchases().then(authUI.onGetPurchases).catch(authUI.onGetFailure);
};

var searchItems = function searchItems() {
  var searchFor = $('input[data-search]').val();
  if (searchFor.length > 0) {
    itemApi.getByName(searchFor).then(itemUI.onIndexSuccess).then(function () {
...
var searchProductId = function searchProductId(event) {
  event.preventDefault();

  var data = getFormFields(event.target);
  // data = {product{id: val}}
  itemApi.getItem(data.product.id).then(function (data) {
...
var deleteProduct = function deleteProduct(event) {
  ...
  itemApi.deleteItem(data.product.id).then(function () {
    ui.product2IdToggle();
...
var addProduct = function addProduct(event) {
  ...
  var data = getFormFields(event.target);
  itemApi.addItem(data).then(function () {
    ui.addProductToggle();
    ui.addSuccess();
  }).catch(ui.addFailure);
};
...
var removeCartItem = function removeCartItem(itemId) {
  itemId = {
    product: {
      id: itemId
    }
...
var clearPasswordFields = function clearPasswordFields() {
  $('input[name="credentials[password]"]').val('');
  $('input[name="credentials[password_confirmation]"]').val('');
};
...
var onGetCartSuccess = function onGetCartSuccess(response, status, xhr) {
...
var onGetCartFailure = function onGetCartFailure(response, status, xhr) {
  $('#alert-modal-content').addClass('alert-danger');
  $('#alert-modal-content').html('<p>Sorry, we could not find your cart.</p>');
  $('#alertModal').modal('show');
};

var onAddToCartSuccess = function onAddToCartSuccess(response, status, xhr) {
  store.user.cart = response.cart;
};

var onAddToCartFailure = function onAddToCartFailure(response, status, xhr) {
  $('#alert-modal-content').addClass('alert-danger');
  $('#alert-modal-content').html('<p>Sorry, we could not add this to your cart.</p>');
  $('#alertModal').modal('show');
};

var updateCartDisplay = function updateCartDisplay() {
  $('#nav-qty').text(store.user.cartItemTotal || 0);
  $('#nav-price').text(store.user.cartItemPrice || 0 .toFixed(2));
};

var onPurchaseSuccess = function onPurchaseSuccess(response, status, xhr) {
  $('#item-view-modal').modal('hide');
  $('#alert-modal-content').addClass('alert-success');
...
var onPurchaseFailure = function onPurchaseFailure(response, status, xhr) {
  $('#alert-modal-content').addClass('alert-danger');
  $('#alert-modal-content').html("<p>We could not process your purchase, but don't worry, your money is safe with us.</p>");
  $('#alertModal').modal('show');
};

var onGetPurchases = function onGetPurchases(response, status, xhr) {
...
module.exports = {
  ...
  onGetCartFailure: onGetCartFailure,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "      <div id=\"accordion\" role=\"tablist\" aria-multiselectable=\"true\">\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.purchases : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "      </div>\n";
  ...
  return "          <div class=\"card\">\n            <div class=\"card-header\" role=\"tab\" id=\"headingTwo\">\n              <h5 class=\"mb-0\">\n          <a class=\"collapsed\" data-toggle=\"collapse\" data-parent=\"#accordion\" href=\"#"
    ...
    + "</p>\n              </div>\n              <div class=\"col-xs-3\">\n                <p class=\"pull-right\">Total items:  "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.totalItems : stack1), depth0))
    + "</p>\n              </div>\n              <div class=\"col-xs-3\">\n                <p class=\"pull-right\">Price:  $"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.totalPrice : stack1), depth0))
    + "</p>\n              </div>\n            </div>\n          </a>\n        </h5>\n            </div>\n            <div id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"collapse\" role=\"tabpanel\" aria-labelledby=\"headingTwo\">\n              <div class=\"card-block\">\n                <div class=\"container-fluid\">\n                  <table class=\"table table-responsive table-hover\">\n                    <thead>\n                      <th>Item</th>\n                      <th>Quantity</th>\n                      <th>Line Price</th>\n                    </thead>\n                    <tbody>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.products : stack1),{"name":"each","hash":{},"fn":container.program(3, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "                    </tbody>\n                  </table>\n                </div>\n              </div>\n            </div>\n          </div>\n";
},"3":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "                        <tr>\n                          <td>"
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1["0"] : stack1)) != null ? stack1.name : stack1), depth0))
    + "</td>\n                          <td>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1["1"] : stack1), depth0))
    + "</td>\n                          <td>$"
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1["0"] : stack1)) != null ? stack1.price : stack1), depth0))
    + "</td>\n                        </tr>\n";
},"5":function(container,depth0,helpers,partials,data) {
    return "      <h5>You have no purchases.</h5>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<div class=\"container-fluid\">\n  <section id=\"title\">\n    <div class=\"jumbotron\">\n      <h1>Nozama</h1>\n      <h3>Account Management</h3>\n    </div>\n  </section>\n  <section id=\"links\">\n    <div class=\"container-fluid\">\n      <div class=\"row\"><a href=\"#\" data-backToShopping>Go back...</a></div>\n    </div>\n    <h3>Password Management</h3>\n    <div class=\"container\">\n\n      <div class=\"container-fluid\">\n        <div class=\"row\"><a href=\"#\" data-changePassword>Change password</a></div>\n      </div>\n    </div>\n    <div class=\"hidden\" id=\"change-password-area\">\n      <form id=\"change-password-form\">\n        <div class=\"row\">\n          <div class=\"form-group col-xs-12 col-sm-4\">\n            <label for=\"passwordLabel\">Current Password</label>\n            <input name=\"passwords[old]\" type=\"password\" class=\"form-control\" placeholder=\"Current Password\" required>\n          </div>\n          <div class=\"form-group col-xs-12 col-sm-4\">\n            <label for=\"passwordLabel\">New Password</label>\n            <input name=\"passwords[new]\" type=\"password\" class=\"form-control\" placeholder=\"New Password\" required>\n          </div>\n        </div>\n        <div class=\"row container-fluid\">\n          <button class=\"btn btn-secondary\" data-cancelChange>Cancel</button>\n          <button type=\"submit\" class=\"btn btn-primary\" data-changeSubmit>Submit</button>\n        </div>\n      </form>\n    </div>\n\n  </section>\n\n  <h3>Your Purchases</h3>\n  <div class=\"container-fluid\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.purchases : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(5, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </div>\n</div>\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <div class=\"jumbotron\">\n    <h1>Nozama</h1>\n    <p>Buy our stuff</p>\n  </div>\n  <div id=\"result\" class=\"product-title\"></div>\n  <div class=\"row\">\n    <div class=\"col-xs-12\">\n      <div class=\"input-group col-xs-4 search-box\">\n        <input type=\"text\" class=\"form-control\" placeholder=\"Search\" data-search>\n        <div class=\"input-group-btn\">\n          <button class=\"btn btn-default\" type=\"submit\" data-search>\n          <i class=\"glyphicon glyphicon-search\"></i>\n        </button>\n        </div>\n      </div>\n    </div>\n  </div>\n  <br/>\n  <div class=\"container-fluid\">\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.products : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </div>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "\n      <div class=\"col-xs-12 col-sm-4 well\">\n        <div class=\"row\">\n          <img src=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.picture_URL : stack1), depth0))
    + "\" id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    ...
    + "\n            <button data-prodID=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn btn-success button-place\" type=\"button\" name=\"button\"> <span class=\"glyphicon glyphicon-shopping-cart\">+</span></button>\n        </div>\n      </div>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "  <h4> Failed to load the store, please try again later</h4>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "<div class=\"modal-header\">\n  <div class=\"row\">\n    <div class=\"col-xs-12\">\n  <h1 class=\"product-title\">"
    ...
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.product : depth0)) != null ? stack1.picture_URL : stack1), depth0))
    + "\" id=\""
    ...
    + "</h5>\n    </div>\n    <div class=\"col-xs-4\">\n      <h4>Price: <span data-price=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.product : depth0)) != null ? stack1.price : stack1), depth0))
    + "\">$"
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.product : depth0)) != null ? stack1.price : stack1), depth0))
    + "</span></h4>\n    </div>\n  </div>\n</div>\n\n<div class=\"modal-footer\">\n    <h4>Quantity: <input class=\"footer-space\" data-itemqty type=\"number\" name=\"quantity\" min=\"1\" max=\"999\" step=\"1\" value=\"1\">\n    Total Price: <span class=\"footer-space\" id=\"qty-price\" data-total-price>$"
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.product : depth0)) != null ? stack1.price : stack1), depth0))
    + "</span>\n    <button data-prodID=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.product : depth0)) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn btn-success\" type=\"button\" name=\"button\"> <span class=\"glyphicon glyphicon-shopping-cart\">+</span></button></h4>\n  </div>\n</div>\n";
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
    ...
    return "class=\"modal-expand\"";
},"5":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, helper, alias1=depth0 != null ? depth0 : (container.nullContext || {}), alias2=helpers.helperMissing, alias3="function", alias4=container.escapeExpression;

  return "  <div class=\"product-title modal-header\">\n    <h2>Your Shopping Cart</h2>\n  </div>\n  <table class=\"table table-responsive table-hover\">\n    <thead>\n      <th>Item</th>\n      <th>Quantity</th>\n      <th>Total cost</th>\n      <th></th>\n      <th></th>\n    </thead>\n    <tbody>\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.cart : depth0),{"name":"each","hash":{},"fn":container.program(6, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </tbody>\n  </table>\n  <div class=\"modal-footer\">\n    <div class=\"pull-left\"><button id=\"empty-cart\" class=\"btn btn-danger\">Empty Cart <span class=\"glyphicon glyphicon-trash\"></span></div>\n    <div class=\"pull-right\">\n\n      <h4><a data-backToShopping href=\"#\" class=\"btn btn-primary\">Continue Shopping</a>    Items in cart: "
    + alias4(((helper = (helper = helpers.total || (depth0 != null ? depth0.total : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"total","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + " Total price: $"
  ...
  return "      <tr>\n        <td>"
    ...
    + "\" min=\"1\" step=\"1\" max=\"999\" data-quantity=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\"></td>\n        <td>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.price : stack1), depth0))
    + "</td>\n        <td>\n          <button class=\"btn btn-sm btn-success\" data-update=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">Update Cart</button>\n        </td>\n        <td>\n          <button class=\"btn btn-sm btn-danger\" data-delete=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">&times;</button>\n        </td>\n      </tr>\n";
},"8":function(container,depth0,helpers,partials,data) {
    return "    <div class=\"modal-content empty-modal\">\n      <h3> Your cart is empty</h3>\n      <img src=\"https://i.imgur.com/DSTCCs6.jpg\"/>\n    </div>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=depth0 != null ? depth0 : (container.nullContext || {});

  return "<div "
    + ((stack1 = helpers["if"].call(alias1,(depth0 != null ? depth0.cart : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(3, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ">\n"
    + ((stack1 = helpers["if"].call(alias1,(depth0 != null ? depth0.cart : depth0),{"name":"if","hash":{},"fn":container.program(5, data, 0, blockParams),"inverse":container.program(8, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</div>\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<div>\n  <div class=\"product-title modal-header\">\n    <h2>Checkout</h2>\n  </div>\n  <div class=\"modal-body\">\n    <div class=\"container-fluid\">\n      <div class=\"row row-vextor\">\n        <div class=\"col-xs-7 no-float\">\n          <table class=\"table table-responsive table-hover\">\n            <thead>\n              <th>Item</th>\n              <th>Quantity</th>\n              <th>Total cost</th>\n            </thead>\n            <tbody>\n"
    + ((stack1 = helpers.each.call(alias1,(depth0 != null ? depth0.cart : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "            </tbody>\n          </table>\n        </div>\n        <div class=\"col-xs-5 no-float vertical-rule-left\">\n          <form id=\"checkout\">\n            <div class=\"input-group\">\n              <span class=\"input-group-addon\">Credit Card</span>\n              <input type=\"text\" name=\"purchase[card]\" class=\"form-control\" placeholder=\"Card Number\" required>\n              <input type=\"text\" name=\"purchase[exp]\" class=\"form-control\" placeholder=\"MM/YY\" required>\n              <input type=\"text\" name=\"purchase[cvc]\" class=\"form-control\" placeholder=\"CVC\" required>\n            </div>\n            <input type=\"Submit\" value=\"Complete Purchase\" class=\"btn btn-success\">\n            <button type=\"button\" class=\"btn btn-default pull-right\" data-dismiss=\"modal\">Cancel</button>\n          </form>\n        </div>\n      </div>\n    </div>\n  </div>\n  <div class=\"modal-footer\">\n    <div class=\"pull-right\">\n      <h4>Total price: $"
    + container.escapeExpression(((helper = (helper = helpers.price || (depth0 != null ? depth0.price : depth0)) != null ? helper : helpers.helperMissing),(typeof helper === "function" ? helper.call(alias1,{"name":"price","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "\n      </h4>\n    </div>\n  </div>\n</div>\n";
},"useData":true,"useBlockParams":true});
...
function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "<div class=\"container-fluid\">\n  <section id=\"title\">\n    <div class=\"jumbotron\">\n      <h1>Nozama</h1>\n      <h3>Products Dashboard</h3>\n    </div>\n  </section>\n  <section id=\"links\">\n    <div class=\"container-fluid\">\n      <!-- TODO: Make me pretty -->\n      <div class=\"row\"><a href=\"#\" data-deleteProduct>Delete Product</a></div>\n      <div class='hidden' id=\"product-id-area2\">\n        <form id=\"product-id-form2\">\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[id]\" type=\"input\" class=\"form-control\" placeholder=\"Product ID\" required>\n            </div>\n            <div class=\"col-xs-12 col-sm-4\">\n              <button class=\"btn btn-secondary\" data-cancelProduct2Id>Cancel</button>\n              <button type=\"submit\" class=\"btn btn-primary\">Submit</button>\n            </div>\n          </div>\n        </form>\n      </div>\n      <div class=\"row\"><a href=\"#\" data-addProduct>Add a Product</a></div>\n      <div class=\"hidden\" id=\"add-product-area\">\n        <form id=\"add-product-form\">\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[name]\" type=\"input\" class=\"form-control\" placeholder=\"Product Name\" required>\n            </div>\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[price]\" type=\"input\" class=\"form-control\" placeholder=\"Product Price\" required>\n            </div>\n          </div>\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[description]\" type=\"input\" class=\"form-control\" placeholder=\"Product Description\" required>\n            </div>\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[picture_URL]\" type=\"input\" class=\"form-control\" placeholder=\"Picture URL\" required>\n            </div>\n          </div>\n          <div class=\"row\">\n            <div class=\"col-xs-12 col-sm-4\">\n              <button class=\"btn btn-secondary\" data-cancelAddProduct>Cancel</button>\n              <button type=\"submit\" class=\"btn btn-primary\" data-submitAddProduct>Submit</button>\n            </div>\n          </div>\n        </form>\n      </div>\n      <div class=\"row\"><a href=\"#\" data-updateProduct>Update Product</a></div>\n      <div class=\"hidden\" id=\"product-id-area\">\n        <form id=\"product-id-form\">\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[id]\" type=\"input\" class=\"form-control\" placeholder=\"Product ID\" required>\n            </div>\n            <div class=\"col-xs-12 col-sm-4\">\n              <button class=\"btn btn-secondary\" data-cancelProductId>Cancel</button>\n              <button type=\"submit\" class=\"btn btn-primary\" data-submitProductId>Submit</button>\n            </div>\n          </div>\n        </form>\n      </div>\n      <div class=\"hidden\" id=\"edit-product-area\">\n        <form id=\"edit-product-form\">\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[name]\" type=\"input\" class=\"form-control\" placeholder=\"Product Name\" required>\n            </div>\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[price]\" type=\"input\" class=\"form-control\" placeholder=\"Product Price\" required>\n            </div>\n          </div>\n          <div class=\"row\">\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[description]\" type=\"input\" class=\"form-control\" placeholder=\"Product Description\" required>\n            </div>\n            <div class=\"form-group col-xs-12 col-sm-4\">\n              <input name=\"product[picture_URL]\" type=\"input\" class=\"form-control\" placeholder=\"Picture URL\" required>\n            </div>\n          </div>\n          <div class=\"row\">\n            <div class=\"col-xs-12 col-sm-4\">\n              <button class=\"btn btn-secondary\" data-cancelEditProduct>Cancel</button>\n              <button type=\"submit\" class=\"btn btn-primary\" data-submitEditProduct>Submit</button>\n            </div>\n          </div>\n        </form>\n      </div>\n    </div>\n</div>\n</section>\n\n<section id=\"purchase-history\"></section>\n</div>\n\n\n<!-- const productSchema = new mongoose.Schema({\n  picture_URL: {\n    type: String,\n    required: true\n  },\n  name: {\n    type: String,\n    required: true\n  },\n  description: {\n    type: String,\n    required: true\n  },\n  price: {\n    type: Number,\n    required: true\n  }\n}, {\n  timestamps: true,\n  toJSON: {\n    virtuals: true\n  }\n}) -->\n";
},"useData":true});
...
var getByName = function getByName(data) {
  return $.ajax({
